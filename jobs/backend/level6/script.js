...
var setModifications = function(rental_id, rentals, modif) {
	for (var i = 0; i < rentals.length; i++) {
		if (rentals[i].id != rental_id) continue;
		for (var prop in modif) {
			if (rentals[i][prop]) rentals[i][prop] = modif[prop];
		}
	}
};
var getRental = function(rental_id, rentals, cars, modif) {
	var rent = {};
    for (var i = 0; i < rentals.length; i++) {
		if (rentals[i].id != rental_id) continue;
		
		if (modif) setModifications(rental_id, rentals, modif);
...
var getMerge = function(rental_id, modif_id, rentals, cars, modif) {
	function getType(who, dif) {
		if (who == "driver") {
			return  dif > 0 ? "debit": "credit";
		}
		else return dif > 0 ? "credit": "debit";
	}
	var delta = {};
	var before = getRental(rental_id, rentals, cars, null);
	var after  = getRental(rental_id, rentals, cars, modif);
	
	delta.id 		= modif_id
	delta.rental_id = rental_id;
	delta.actions = [];
	
	var j = 0;
	for (var i = 0; i < before.actions.length; i++) {
		var dif = after.actions[i].amount - before.actions[i].amount;
		var who = before.actions[i].who;
		
		delta.actions[j++] = { "who": who, 
							   "type": getType(who, dif),
							   "amount": Math.abs(dif),
							 };
	}
	return delta;
...
var main = function(text) {
	...
	var rentals = obj_in.rentals;
	var cars    = obj_in.cars;
	
	var rental_modif = [];
	rental_modif = obj_in.rental_modifications;
	
	var obj_out = {};
	obj_out.rental_modifications = [];
	
	var j = 0;
	for (var i = 0; i < rental_modif.length; i++) {
		var rental_id = rental_modif[i].rental_id;
		var modif_id = rental_modif[i].id;
		
		var modif = {};
		for (var prop in rental_modif[i]) {
			if (prop == "rental_id") continue;
			if (prop == "id") 		 continue;
			modif[prop] = rental_modif[i][prop];
		}
		obj_out.rental_modifications[j++] = getMerge(rental_id, modif_id, rentals, cars, modif);
	}
