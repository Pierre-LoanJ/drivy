...
var main = function(text) {
    ...
    for (var i = 0; i < obj_in.rentals.length; i++) {
		...
		var isOK = true;
		if (tComp < 0) {
			console.log("Error: tComp must be positive, tComp=", tComp);
			isOK = false;
		}
        if (dComp < 0) {
			console.log("Error: dComp must be positive, dComp=", dComp);
			isOK = false;
		}
		var rentalPrice, insurCom, assistCom, drivyCom;
		if (isOK) {
			rentalPrice = tComp + dComp;
		...
		if (isOK) {
			obj_out.rentals[j++] = {"id"   	 	: rentalId,
									"price"	 	: tComp + dComp, 
									"commission": {
										"insurance_fee" : insurCom,
										"assistance_fee": assistCom, 
										"drivy_fee"  : drivyCom
										}
									};
		}
